...
Cypress.Commands.add("addClaim", (claimContent) => {
    cy.visit(Cypress.env('baseURL') + '/cybersecurity-goal').then(() => {
        ...
        cy.get(cybersecurityPoolSelector.goalPoolClaimPoolTabDiv).click().then(() => {
            cy.get(cybersecurityPoolSelector.claimPoolAddNewClaimButton).click().then(() => {
                recurse(
...
Cypress.Commands.add("addControl", (controlContent) => {
    cy.visit(Cypress.env('baseURL') + '/cybersecurity-control').then(() => {
        cy.get(cybersecurityPoolSelector.controlPoolAddNewControlButton).click().then(() => {
            recurse(
...
Cypress.Commands.add("ImportControlFromLibrary", (controlName) => {
    cy.visit(Cypress.env('baseURL') + '/cybersecurity-control').then(() => {
        cy.get(cybersecurityPoolSelector.controlPoolAddNewControlButton).click({ force: true }).then(() => {
            cy.get(cybersecurityPoolSelector.cybersecurityPoolAddFromLibraryParagraph).click().then(() => {
                recurse(
                ...
                cy.get(cybersecurityPoolSelector.cybersecurityPoolListOption).should('have.length', 1).then(() => {
                    cy.get(cybersecurityPoolSelector.cybersecurityPoolListOption).should('include.text', controlName).then(() => {
...
Cypress.Commands.add("addRequirementToLibrary", (requirementName) => {
    cy.visit(Cypress.env('baseURL') + '/library').then(() => {
        cy.get(projectLibrarySelector.librarySideNavRequirementAnchor).click({ force: true }).then(() => {
            cy.get(projectLibrarySelector.projectLibraryCreateNewButton).click({ force: true }).then(() => {
...
Cypress.Commands.add("addRequirement", (requirementName) => {
    cy.visit(Cypress.env('baseURL') + '/requirements').then(() => {
        cy.get(requirementPoolSelector.requirementPoolAddNewButton).click().then(() => {
            cy.get(requirementPoolSelector.newReqDialogDescriptionTypeInput).click().then(() => {
                cy.get(requirementPoolSelector.newReqDialogNextButton).click();
                recurse(
                    () => cy.get(requirementPoolSelector.newReqDialogDescriptionTextArea).clear().type(requirementName),
                    ($inputField) => $inputField.val() === requirementName,
    ...
    cy.visit(Cypress.env('baseURL') + '/library').then(() => {
        cy.get(projectLibrarySelector.librarySideNavControlAnchor).click({ force: true }).then(() => {
            ...
            cy.get(cybersecurityPoolSelector.poolRecordContentTextArea).each(($element) => {
                ...
                cy.get(projectLibrarySelector.moreOptionsDeleteControlButton).click().then(() => {
    ...
    cy.visit(Cypress.env('baseURL') + '/library').then(() => {
        cy.get(projectLibrarySelector.librarySideNavGoalClaimAnchor).click({ force: true }).then(() => {
            ...
            cy.get(cybersecurityPoolSelector.poolRecordContentTextArea).each(($element) => {
                ...
                cy.get(projectLibrarySelector.moreOptionsDeleteClaimButton).click().then(() => {
                    cy.get(navBarSelector.confirmDialogueDeleteButton).last().click({ force: true });
                    cy.wait(1000);
                })
            })
