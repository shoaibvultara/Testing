...
describe('Threat List View Drop Down Actions', () => {
    ...
    it('Verify the working of "Threat History" button (MAIN-TC-840)', () => {
        cy.visit(Cypress.env('baseURL') + '/threats').then(() => {
            cy.get(threatListViewSelector.threatListViewAttackPathColumn).should('exist').then(() => {
                recurse(
                ...
                cy.get(threatListViewSelector.threatListViewAttackPathColumn).then(($foundElementList) => {
                    ...
                    cy.threatHistory(threatCount).then(() => { //last threat
    ...
    it('Verify the "Add new threat" button functionality (MAIN-TC-1366)', () => {
        cy.visit(Cypress.env('baseURL') + '/threats').then(() => {
            ...
            cy.addNewThreat().then(() => {
                ...
                cy.get('@postRequest').its('response.statusCode').should('eq', 200).then(() => {
    ...
    it('Verify if user Permanently Deleted a Threat, it should not generate again when the model is run and, should exist in restore threat dialog(MAIN-TC-2147, MAIN-TC-1209)', () => {
        ...
        let threatId;
        cy.visit(Cypress.env('baseURL') + '/threats').then(() => {
            cy.get(threatListViewSelector.threatListViewThreatScenarioTextArea).first().invoke('val').then(($value) => {
                scenario = $value;
            });
            cy.get(threatListViewSelector.threatListViewThreatNoTableData).first().invoke('text').then(($text) => {
                threatId = $text;
            });
            cy.deleteThreat('permanent', '1').then(() => {
            ...
            cy.get(navBarSelector.navBarEditButton).click().then(() => {
                cy.get(navBarSelector.editListRestoreThreatButton).click().then(() => {
