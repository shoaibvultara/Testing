        ...
        cy.addGoal(goalContent).then(() => {
            cy.visit(Cypress.env('baseURL') + '/threats').then(() => {
                ...
                cy.changeThreatTreatment(threatRow, 'reduce').then(() => {
                    ...
                    cy.get(threatListViewSelector.globalDropDownList).contains(goalContent).click().then(() => {
                        recurse(
                            ...
                            recurse(
                                ...
                                cy.get(navBarSelector.confirmDialogueConfirmButton).click().then(() => {
                                    ...
                                    cy.get(threatListViewSelector.threatListViewExpandedRecordContent).should('contain', updatedGoalDescription).then(() => {
        ...
        cy.addClaim(claimContent).then(() => {
            cy.visit(Cypress.env('baseURL') + '/threats').then(() => {
                ...
                cy.changeThreatTreatment(threatRow, 'retain').then(() => {
                    cy.get(threatListViewSelector.threatListViewExpandedThreatAddIcon)
                        .should('have.length', 3).last().click({ force: true });//1st add goal, 2nd add control
                }).then(() => {
                    cy.get(threatListViewSelector.threatListViewAddNewClaimFromPool).click();
                    cy.get(threatListViewSelector.globalDropDownList).contains(claimContent).click().then(() => {
                        recurse(
                            ...
                            recurse(
                                ...
                                cy.get(navBarSelector.confirmDialogueConfirmButton).click().then(() => {
                                    ...
                                    cy.get(threatListViewSelector.threatListViewExpandedRecordContent).should('contain', updatedClaimDescription).then(() => {
                                        cy.visit(Cypress.env('baseURL') + '/threats');
                                        cy.get(threatListViewSelector.threatListShowFilterButton).click();
                                        cy.get(threatListViewSelector.filterByGoalAndClaimButton).click().then(() => {
